...
public class RegistrationResponseProcessing {
	...
	//code added for IOS support
	public RegistrationRecord[] processIOSResponse(RegistrationResponse response)
			throws Exception {
		checkAssertions(response);
		RegistrationRecord[] records = new RegistrationRecord[response.assertions.length];
		...
		checkFcp(fcp);
		for (int i = 0; i < records.length; i++) {
			records[i] = processIOSAssertions(response.assertions[i], records[i]);
		}
	...
	//code added for IOS support
	private RegistrationRecord processIOSAssertions(
			AuthenticatorRegistrationAssertion authenticatorRegistrationAssertion,
			RegistrationRecord record) {
		if (record == null) {
			record = new RegistrationRecord();
		...
		try {
			Tags tags = parser
					.parse(authenticatorRegistrationAssertion.assertion);
			try {
				verifyIOSAttestationSignature(tags, record);
			} catch (Exception e) {
				record.attestVerifiedStatus = "NOT_VERIFIED";
			}
	...
	//Code added for IOS support
	private void verifyIOSAttestationSignature(Tags tags, RegistrationRecord record)
			throws NoSuchAlgorithmException, IOException, Exception {
		byte[] certBytes = tags.getTags().get(TagsEnum.TAG_ATTESTATION_CERT.id).value;
		System.out.println("\ncert bytes:" +new String(certBytes));
		
		//====================================================================================================
		/**for Ios, decode once to form the format to be passed for X509 cert class
		Decoding first time and storing the value in registration record [Code added by me]*/
		//byte[] decodeCertBytes1 = Base64.decodeBase64(certBytes);
		//System.out.println("\n\nDecoded certbytes 1st time (Actual Certificate): " +new String(decodeCertBytes1));
		//====================================================================================================
		
		//String encodedCert = Base64.encodeBase64URLSafeString(certBytes);
		//byte[] decodedCertBytes2 = Base64.decodeBase64(encodedCert);
		//record.attestCert = new String(decodedCertBytes2);
		record.attestCert = Base64.encodeBase64URLSafeString(certBytes);
	...
	private void checkFcp(FinalChallengeParams fcp) throws Exception {
		...
		/*RegistrationRequest regRequest = (RegistrationRequest) Dash.getInstance().stats.get(Dash.LAST_REG_REQ);
		//Validate App Id
		if(regRequest.header.appID.equals(appId)){
			System.out.println("\nFCP: App ID matched!!!");
		}
		else{
			System.out.println("\nFCP: App ID not matched!!!");
			throw new Exception();
		}
		
		//Validate Challenge
		if(regRequest.challenge.equals(challenge)){
			System.out.println("\nFCP: Challenge ID matched!!!");
		}
		else{
			System.out.println("\nFCP: Challenge ID not matched!!!");
			throw new Exception();
		}*/
		
		//Validate Facet Id
		/*if(){
			System.out.println("\nFCP: Facet ID matched!!!");
		}
		else{
			System.out.println("\nFCP: Facet ID not matched!!!");
			throw new Exception();
		}*/
