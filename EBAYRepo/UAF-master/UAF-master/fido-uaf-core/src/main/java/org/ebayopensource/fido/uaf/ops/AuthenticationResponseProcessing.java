...
public class AuthenticationResponseProcessing {
	...
	private AuthenticatorRecord processAssertions(
		...
		try {
			...
			/** commented below code for Database support */
	...
	private boolean verifySignature(Tag signedData, Tag signature,
		...
		// Store Encoding algorithm in a Map to be used later on for context
		// verification during flexible authentication
		//algorithmStore.put("algorithm", algAndEncoding);
		...
		System.out
				.println("\ndecoded base 64 public key (2nd time to sent to x509 spec)"
		...
		if (algAndEncoding == AlgAndEncodingEnum.UAF_ALG_SIGN_RSASSA_PSS_SHA256_RAW) {
			...
			PublicKey publicKey = KeyCodec.getPublicKey(new DEROctetString(
					decodeBase64).getOctets());
			return RSA.verifySimpleRSA(publicKey, dataForSigning,
					signature.value);
		} else {
			if (algAndEncoding == AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256K1_ECDSA_SHA256_DER) {
				ECPublicKey decodedPub = (ECPublicKey) KeyCodec
						.getPubKeyFromCurve(decodeBase64, "secp256k1");
			...
			if (algAndEncoding == AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256R1_ECDSA_SHA256_DER) {
				if (decodeBase64.length > 65) {
					...
					ECPublicKey decodedPub = (ECPublicKey) KeyCodec
							.getPubKeyFromCurve(decodeBase64, "secp256r1");
	...
	private RegistrationRecord getRegistrationRecordFrmDB(
			AuthenticatorRecord authRecord, StorageInterface serverData)
			throws IOException {
		return serverData.readRegistrationRecordfromDB(authRecord.toString());
	}
	...
	/**
	 * Method that decrypts and verifies the context for validation
	 * @param signedData
	 * @param signature
	 * @param pubKey
	 * @param accountId
	 ...
	 * @throws Exception
	 */
	public static boolean verifyContextSignatures(String signedData, String signature,
			String pubKey) throws InvalidKeyException, NoSuchAlgorithmException,
			NoSuchProviderException, SignatureException,
			UnsupportedEncodingException, Exception {
		
		//Hardcode Sign and Verification encoding Algorithm for now
		Map<String, AlgAndEncodingEnum> algorithmStore = new HashMap<String, AlgAndEncodingEnum>();
		algorithmStore.put("algorithm", AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256R1_ECDSA_SHA256_RAW);
		

		byte[] dataForSigning = signedData.getBytes();
		byte[] signatures = signature.getBytes();
		...
		System.out
				.println("\ndecoded base 64 public key (2nd time to sent to x509 spec)"
						+ new String(decodeBase64));
		if (algorithmStore.get("algorithm") == AlgAndEncodingEnum.UAF_ALG_SIGN_RSASSA_PSS_SHA256_RAW) {
			PublicKey publicKey = KeyCodec.getRSAPublicKey(decodeBase64);
			return RSA.verifyPSS(publicKey, SHA.sha(dataForSigning, "SHA-256"),
					signatures);
		} else if (algorithmStore.get("algorithm") == AlgAndEncodingEnum.UAF_ALG_SIGN_RSASSA_PSS_SHA256_DER) {
			/** commented below code for IOS (RSA keys) compatibility */
			// PublicKey publicKey = KeyCodec.getRSAPublicKey(new
			// DEROctetString(decodeBase64).getOctets());
			// return RSA.verifyPSS(publicKey, SHA.sha(dataForSigning,
			...
			PublicKey publicKey = KeyCodec.getPublicKey(new DEROctetString(
					decodeBase64).getOctets());
			return RSA.verifySimpleRSA(publicKey, dataForSigning, signatures);
		} else {
			if (algorithmStore.get("algorithm") == AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256K1_ECDSA_SHA256_DER) {
				ECPublicKey decodedPub = (ECPublicKey) KeyCodec
						.getPubKeyFromCurve(decodeBase64, "secp256k1");
				return NamedCurve.verifyUsingSecp256k1(
						KeyCodec.getKeyAsRawBytes(decodedPub),
						SHA.sha(dataForSigning, "SHA-256"),
						Asn1.decodeToBigIntegerArray(signatures));
			}
			if (algorithmStore.get("algorithm") == AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256R1_ECDSA_SHA256_DER) {
				if (decodeBase64.length > 65) {
					return NamedCurve.verify(KeyCodec.getKeyAsRawBytes(pubKey),
							SHA.sha(dataForSigning, "SHA-256"),
							Asn1.decodeToBigIntegerArray(signatures));
			...
			if (signatures.length == 64) {
				...
				return NamedCurve.verify(KeyCodec.getKeyAsRawBytes(decodedPub),
						SHA.sha(dataForSigning, "SHA-256"),
						Asn1.transformRawSignature(signatures));
			} else if (65 == decodeBase64.length
					&& AlgAndEncodingEnum.UAF_ALG_SIGN_SECP256R1_ECDSA_SHA256_DER == algorithmStore
							.get("algorithm")) {
				ECPublicKey decodedPub = (ECPublicKey) KeyCodec
						.getPubKeyFromCurve(decodeBase64, "secp256r1");
				return NamedCurve.verify(KeyCodec.getKeyAsRawBytes(decodedPub),
