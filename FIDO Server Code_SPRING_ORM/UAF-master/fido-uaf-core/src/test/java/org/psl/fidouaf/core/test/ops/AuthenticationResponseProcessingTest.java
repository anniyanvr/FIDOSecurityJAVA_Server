...
public class AuthenticationResponseProcessingTest {
	...
	public void basic() throws Exception {
		...
		List<AuthenticatorRecord> authenticatorRecords = p.verify(response,
				serverData);
		assertTrue(authenticatorRecords.size() == 1);
		assertEquals(authenticatorRecords.get(0).username,
				Constants.TEST_USERNAME);
		assertEquals(authenticatorRecords.get(0).status, "SUCCESS");

	}

	private AuthenticationResponse getTestResponse() {
		return gson.fromJson(getTestResponseAsJsonString(),
	...
	class ServerDataImpl implements StorageInterface {
		...
		public void store(List<RegistrationRecord> records) {
			// TODO Auto-generated method stub
		...
		public RegistrationRecord readRegistrationRecord(String key) {
			...
			r.username = Constants.TEST_USERNAME;
			...
			// r.PublicKey =
			// "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEKJFyB4AHNtBlqc555yF_Xg9m7SvkfrllBYDirfJdu0XK6zq6ieurgjHzrhuCNbrFFDmu3_wNfb--BQvh7kgedA==";
			return r;
		}
