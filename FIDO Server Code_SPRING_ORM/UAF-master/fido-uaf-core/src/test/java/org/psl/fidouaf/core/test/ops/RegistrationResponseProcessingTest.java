...
public class RegistrationResponseProcessingTest {
	...
	public void test() throws Exception {
		...
		List<RegistrationRecord> processResponse = rrp
				.processResponse(getResponseWithLongerSignature());
		assertTrue(processResponse.size() == 1);
		logger.info("AAID=" + processResponse.get(0).authenticator.AAID);
	}

	@Test
	public void serverDataValidation() throws Exception {
		RegistrationResponseProcessing rrp = new RegistrationResponseProcessing(
				5 * 60 * 1000, new NotaryImpl());
		...
		assertTrue(processResponse.size() == 1);
		logger.info("AAID="
				+ processResponse.get(0).getAuthenticator().getAAID());
	}

	private String prepareServerData() {
		return generateServerData("testUsername", "challengeString",
				new NotaryImpl());
	}
