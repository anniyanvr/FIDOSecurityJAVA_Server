...
public class RegAssertionBuilder {
	...
	private byte[] getSignedData() throws IOException, InvalidKeySpecException,
	...
	private byte[] getPubKeyId() throws InvalidKeySpecException,
			NoSuchAlgorithmException, NoSuchProviderException, IOException {
		pub = KeyCodec.getPubKey(Base64.decodeBase64(Constants.TEST_PUB_KEY));
	...
	private byte[] getSignature(byte[] dataForSigning) throws Exception {
		...
		pub = KeyCodec.getPubKey(Base64.decodeBase64(Constants.TEST_PUB_KEY));
		priv = KeyCodec
				.getPrivKey(Base64.decodeBase64(Constants.TEST_PRIV_KEY));
		...
		boolean verify = NamedCurve.verify(
				KeyCodec.getKeyAsRawBytes(Constants.TEST_PUB_KEY),
				dataForSigning,
